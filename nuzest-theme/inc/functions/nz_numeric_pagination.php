...
function nz_numeric_pagination()
{
    ...
    if (get_previous_posts_link()) {
        printf('<li class="arrow arrow-left">%s</li>' . "\n", get_previous_posts_link('<span class="prev page-numbers">></span>'));
    }
    ...
    foreach ((array) $links as $link) {
		...
		$class2 = $paged == $link ? ' class="current"' : '';
        printf('<li%s><a href="%s" '.$class2.'>%s</a></li>' . "\n", $class, esc_url(get_pagenum_link($link)), $link);
    }
    ...
    if (get_next_posts_link()) {
        printf('<li class="arrow arrow-right">%s</li>' . "\n", get_next_posts_link('<span class="next page-numbers">></span>'));
    }
