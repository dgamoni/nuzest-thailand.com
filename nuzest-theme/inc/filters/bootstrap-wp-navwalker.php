	...
	public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		...
		if ( strcasecmp( $item->attr_title, 'divider' ) == 0 && $depth === 1 ) {
			...
			*/
			...
			// Nuzest Mega Menu class addition //
			if($args->has_children && get_field('nz_mega_menu', $item)) { $class_names .= ' nz-mega-menu'; }
 			if(get_fields($item->ID)['nz_menu_style'] == "menu_style_1"){ $class_names .=' menu_style_1'; }
			if(get_fields($item->ID)['nz_menu_style'] == "menu_style_2"){ $class_names .=' menu_style_2'; }
			...
			if ( $args->has_children && $depth === 0 ) {
				...
				$atts['class']			= 'nav-link dropdown-toggle '. $item->ID;
			...
			if ($item_fields){
				//$imageUrl = $item_fields['nz_item_image']['url'];
				$image = get_field('nz_item_image', $item->ID);
				$url = $image['url'];
				$size = 'thumbnail';
				$imageUrl = $image['sizes'][ $size ];
				if(!empty($imageUrl)){
					$item_output .= "<div class='hidden-xs hidden-sm ". $item->ID ." meganav-prod-img'><img src='".$imageUrl. "' width = 150></div>";
				}
				}
